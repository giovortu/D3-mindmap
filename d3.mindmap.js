 ...
 */d3.mindMap = function(options){
     /*JSON objects clone method*/if (typeof JSON.clone !== "function"){
    ...
    var rectWidth = 120, rectHeight = 20, scale = 1, linkWeight = 2;
    shiftClicked = false, ctrlClicked = false, fontSize = 10;/*parameters*/
    var _width = (options && options.width) || 960, _height = (options && options.height) || 500, initialDistance = (options && options.initialDistance) || 150, textXMargin = (options && options.textXMargin) || 10, textYMargin = (options && options.textYMargin) || 10, enableEdit = (options && options.editable), containerID = (options && options.container) || "cont", resultsID = (options && options.results) || "result", fixedNodes = (options && options.fixedNodes), nodesData = (options && options.nodes) || {"nodes":[], "viewport": {"scale": 1, "translate":[0, 0]}};
    var margin = {top: 20, right: 20, bottom: 30, left: 40 }, width = _width;//- margin.left - margin.right,
    height = _height;// - margin.top - margin.bottom;
       /*text element text wrapping*/

    function wrap(text, width){
        text.each(function(){
            var text = d3.select(this), words = text.attr("title").split(/\r?\n/);
            interline = 10;
    ...
    function edit(elm, d, i, _x, _y, rx, ry){
        if (d){
            ...
            if (tt){
                ...
                var removed = false;
                form = frm .append("xhtml:form").on("submit", function(){
                    d3.event.preventDefault();
                    return null;
                }).classed("rename-form", true);
                form.append("span").text("Titolo");
                title = form .append("textarea").classed("d3-mindmap-node-editor-title", true);
                title .attr("placeholder", "Inserire il titolo...").attr("value", function(){
                    this.focus();
                    title[0][0].value = txt;
                    return txt;
                });
                form.append("span").text("Contenuto");
                inp = form .append("textarea").attr("placeholder", "Inserire il contenuto...").classed("rename-form", true);
                ...
                canc = buttContainer.append("xhtml:button").classed("d3-mindmap-little-button", true).text("Annulla").on("click", function(){
                    removed = true;
                    d3.selectAll("div#d3-mindmap-node-editor").remove();
                });
                ok = buttContainer.append("xhtml:button").classed("d3-mindmap-little-button", true).text("Salva").on("click", submit);
                frmWidth = frm.node().getBoundingClientRect().width;
                frmHeight = frm.node().getBoundingClientRect().height;
                if (posX + frmWidth > ww)posX = ww - frmWidth;
                if (posY + frmHeight > hh)posY = hh - frmHeight;
                frm.attr("style", "left:" + posX + "px; top:" + posY + "px;");
            }
        }
    }
    var mainMenu =[
    {title: 'Add new node', action: function(elm, d, i, _x, _y, rx, ry){
        ...
        newNode.x = rx;
        newNode.y = ry;
        if (fixedNodes)newNode.fixed = true;
        else delete newNode.fixed;
    ...
    {title: 'Expand all', action: function(elm, d, i){
        expand(root);
        update(root);
    }
    },

    {title: 'Collapse all', action: function(elm, d, i){
        collapse(root);
        update(root);
    }
    }];
    var input;
    var menu =[
    {title: 'Add sub node', action: function(elm, d, i, _x, _y, rx, ry){
        if (d){
            var nn = findNode(d.id, root);
            if (nn.isLeaf){
                nn.children =[];
    ...
    {title: 'Edit node', action: function(elm, d, i, _x, _y, rx, ry){
        edit(elm, d, i, _x, _y, rx, ry);
    }
    },

    {title: 'Remove node', action: function(elm, d, i){
        if (d){
    ...
    var emptyNode = {"name": "New node", "isLeaf": true, "url": "" };
    ...
    var zoomControls = div.append("div").attr("id", "d3-mindmap-zoom-controls");/*var helpControls = div.append( "div" ).attr("id", "d3-mindmap-help-controls" );
    ...
    var zoomPlus = zoomControls.append("button").text("Zoom +").classed("d3-mindmap-little-button", true).attr("id", "zoom_in").on("click", zoomClick);
    var zoomReset = zoomControls.append("button").text("Reset").classed("d3-mindmap-little-button", true).attr("id", "zoom_reset").on("click", zoomClick);
    var zoomMinus = zoomControls.append("button").text("Zoom -").classed("d3-mindmap-little-button", true).attr("id", "zoom_out").on("click", zoomClick);
    if (enableEdit){
    ...
    var zoom = d3.behavior.zoom().scaleExtent([0.2, 2.5]).x(x).y(y).center([width / 2, height / 2]).size([width, height]);
    if (nodesData && nodesData.viewport && nodesData.viewport.scale)zoom.scale(nodesData.viewport.scale);
    if (nodesData && nodesData.viewport && nodesData.viewport.translate)zoom.translate(nodesData.viewport.translate);
    zoom.on("zoom", translateandrescale);
    var svg = d3.select("div#" + containerID).append("svg").attr("id", "d3-mindmap-svg-root").classed("d3-mindmap-svg", true).attr("width", _width).attr("height", _height).attr("pointer-events", "all").append("g")//  .attr("transform", "translate(" + margin.left + "," + margin.top + ")")
       .call(zoom);
    svg.append("g").attr("class", "d3-x-axis").attr("transform", "translate(0," + height +")").call(xAxis);
    svg.append("g").attr("class", "d3-y-axis").call(yAxis);
    rect = svg.append("g").attr("width", width).attr("height", height);/*link arrow*/
    svg.append("svg:defs").selectAll("marker").data(["end"]) // Different link/path types can be defined here
       .enter().append("svg:marker") // This section adds in the arrows
       .attr("id", String).attr("viewBox", "0 -5 10 10").attr("refX", 25).attr("refY", 0).attr("markerWidth", 6).attr("markerHeight", 6).attr("orient", "auto").append("svg:path").attr("d", "M0,-5L10,0L0,5L0,-5").attr("stroke-width", 1).attr("stroke", "#c0c0c0").attr("fill", "#606060");
    var link = rect.selectAll(".d3-mindmap-link");
    var node = rect.selectAll(".d3-mindmap-node");
    translateandrescale(nodesData.viewport);
    update(root);
    var flroot;/* pan and zoom */
    ...
    function update(root){
        ...
        flroot = nodes;
        updateResult();// Restart the force layout.
        force .nodes(nodes).links(links).linkDistance(fontSize * 10).charge( - 8000).gravity(.05).friction(0.0).size([width, height]).start();// Update links.
        link = link.data(links, function(d){
            return d.target.id;
        });
        link.exit().remove();
        link.enter().insert("path",".d3-mindmap-node").classed("d3-mindmap-link", true).style("stroke-width", linkWeight).attr("marker-end", "url(#end)");
        var drag = force.drag();
        if (enableEdit){
            drag .on("drag", nodedrag).on("dragstart", dragstart).on("dragend", dragend);
        }
        // Update nodes.
        ...
        }).attr("class", "d3-mindmap-node").on("click", onclick).on("touchstart", onclick).on("mouseover", mouseover).on("mouseout", mouseout);
        if (enableEdit){
            nodeEnter.call(drag);
        }
        var rect = nodeEnter.append("rect").attr("x", 0).attr("y", 0).attr("height", rectHeight).attr("width", rectWidth).attr("id", function(d){
        ...
        var txt = nodeEnter.append("text").attr("class", "text").style("text-anchor", "middle").attr("id", function(d){
        ...
        var circles = nodeEnter.append("g").attr("class", "textcontentcircle");
        var circle = circles.append("circle").style("fill", "#50e050").style("stroke", "#505020").attr("cx", 0).attr("cy", 0).attr("r", 10).attr("id", function(d){
            return "JIKU_MM_CIRCLE_" + d.id;
        }).on("mouseover", onContent).on("mouseout", outContent);
        var sym = circles.append("text").style("text-anchor", "middle").style("font-size", "15px").style("font-style", "italic").attr("x", - 1).attr("y", 5).attr("width", 12).attr("height", 12).text("i");
        ...
        gs[0].forEach(function(elem){
            ...
            _rect = childs[0];
            _text = childs[1];
            fontSize = computedStyle(d3.select(_text).node(), "fontSize") || "10";//FF
            radius = parseInt(computedStyle(d3.select(_rect).node(), "border-radius") || "20");
            _rect.setAttribute("rx", radius);
            _rect.setAttribute("ry", radius);
            ...
            _circles.attr("transform", "translate(" + w / 2 +"," + (h / 2) +")").attr("visibility", function(d, i){
                if (d.textcontent)return "visible";
                return "hidden";
            });
        });
    }
    ...
    function filterAll(_currentNode){
        ...
        function _filter(currentNode){
            ...
            if (hasValue(currentNode)){
                ...
                if (currentNode.children) parseChilds(currentNode.children);//if (currentNode._children) parseChilds(currentNode._children);
    ...
    function findNode(id, currentNode){
        ...
        if (Array.isArray(currentNode)){
            for (var i = 0;i < currentNode.length;i++){
                elem = currentNode[i];// console.log("recurring");
                ...
                if (hasValue(currentNode)){
                    if (hasValue(currentNode.children)){
                        for (var i = 0;i < currentNode.children.length;i += 1){
                            currentChild = currentNode.children[i];// Search in the current child
                            result = findNode(id, currentChild);// Return the result if the node has been found
                            if (result !== false){
                                return result;
                            }
                        }
    ...
    function flatten(root){
        ...
        function recurse(node){
            node.isLeaf = true;
            if (fixedNodes)node.fixed = true;
            else delete node.fixed;
            if (node.children){
                node.size = node.children.reduce(function(p, v){
                     //v.parent = node ;
                    return p + recurse(v);
    ...
    function zoomClick(){
        var clicked = d3.event.target, direction = 1, factor = 0.2, target_zoom = 1, center =[width / 2, height / 2], extent = zoom.scaleExtent(), translate = zoom.translate(), translate0 =[], l =[], view = {x: translate[0], y: translate[1], k: zoom.scale()};
        d3.event.preventDefault();
        direction = (this.id === 'zoom_in') ? 1: ((this.id === 'zoom_out') ? - 1: 0);
        if (direction == 0)target_zoom = 1;
        else target_zoom = zoom.scale() * (1 + factor * direction);
    ...
    function computedStyle(el, prop, getComputedStyle, style){
        ...
        style = // If we have getComputedStyle
